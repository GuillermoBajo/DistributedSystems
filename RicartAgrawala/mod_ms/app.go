...
func launchApp(remoteUser, addr string, pid int, wg *sync.WaitGroup) {
	...
	cmd := exec.Command("ssh", remoteUser+"@"+ip[0], 
		"cd ./tercero/dist/practica2; export PATH=$PATH:/usr/local/go/bin; go mod tidy;",
...
func main() {
	...
	for i, ip := range ips {
		...
		// cmd := exec.Command("cd /home/alvarodefrancisco/universidad/distribuidos/_practicas/p2; go run main.go ", strconv.Itoa(i+1), wkPath)
	}
